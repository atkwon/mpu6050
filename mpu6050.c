...
int mpu6050_init(mpu6050_handle_t *handle, mpu6050_config_5 *config)
{
	if (handle->address == 0){
	...
	gyro_config_buf[1] = config->gyro_fs << 3; /* uses bits 4,3 */
	ret		   = i2c_write_blocking(handle->i2c, handle->address, gyro_config_buf, 2, false);
	if (ret != 2) {
	...
	accel_config_buf[1] = config->accel_fs << 3;
	ret		    = i2c_write_blocking(handle->i2c, handle->address, accel_config_buf, 2, false);
	if (ret != 2) {
